...
for PATCH in "$HERE"/patches/*.patch; do
  # Use plain `patch` (not `git apply`) so it works whether or not the keymint
  # submodule is a git repo. Idempotent: skip if it already reverse-applies.
  # --fuzz=5 tolerates context drift when the pinned submodule is slightly newer.
  if patch -p1 -d "$ROOT/third_party/keymint" -R --dry-run -f < "$PATCH" >/dev/null 2>&1; then
