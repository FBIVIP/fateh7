...
The control daemon drives it in a loop: `inject <pid> <library.so> entry`, re-running each time the keystore daemon's pid changes (it restarts often, and each restart drops our library). The library it loads is `libfateh7_keymint.so` or `libfateh7_keystore.so`; both export `entry(void*)`.
