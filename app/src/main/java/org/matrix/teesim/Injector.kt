 ...
 * libfateh7_keymint.so; on 10/11 it is keystore with libfateh7_keystore.so. Re-injects whenever the
...
class Injector(private val moduleDir: File) {
    ...
    private val libName = if (api >= 31) "libfateh7_keymint.so" else "libfateh7_keystore.so"
