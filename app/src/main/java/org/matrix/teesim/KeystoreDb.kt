 ...
 *
 * "Ours" is decided by fact, not heuristics: our KeyMint TA prepends the routing marker `FATEH7km\0` to
...
object KeystoreDb {
    ...
    // The routing marker teesim-km prepends to every key blob (`b"FATEH7km\x00"`), as a SQLite blob
    // literal so `substr(blob,1,9) = X'…'` selects the keys we minted.
    private const val MARKER_HEX = "4641544548376B6D00"
